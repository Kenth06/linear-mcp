...
export default function register(server: McpServer, env: Env) {
	server.tool(
		"linearWebhookCreate",
		"Create a Linear webhook (optionally scoped to a team).",
		{
		...
		async (input) => {
			...
			if (input.teamKey && !input.allPublicTeams) {
				...
				}>(env, GQL.teamIdByKey, { key: input.teamKey });
			...
			return {
				content: [
					{ type: "text", text: `Webhook id=${w.id} enabled=${w.enabled}` },
				],
			};
		},
	);
